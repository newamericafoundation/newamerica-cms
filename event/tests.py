...
class EventTests(WagtailPageTests):
	...
	def test_event_parent_page(self):
		self.assertAllowedParentPageTypes(Event, {ProgramEventsPage, AllEventsHomePage})
	...
	def test_event_has_relationship_to_two_parent_programs(self):
		...
		relationship.save()
		self.assertEqual(event.parent_programs.filter(title='Education').first().title, 'Education')
	...
	def test_can_delete_event_with_one_program(self):
		...
		self.assertEqual(PostProgramRelationship.objects.filter(post=event).first(), None)
		self.assertEqual(PostProgramRelationship.objects.filter(post=event, program=self.program_page_1).first(), None)
	...
	def test_can_delete_event_with_two_programs(self):
		...
		self.assertNotIn(event, ProgramEventsPage.objects.filter(title='OTI Events').first().get_children())
		self.assertEqual(PostProgramRelationship.objects.filter(post=event, program=self.program_page_1).first(), None)
		self.assertEqual(PostProgramRelationship.objects.filter(post=event, program=second_program).first(), None)
