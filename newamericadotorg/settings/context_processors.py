...
def program_data(request):
    ...
    if programdata is None:
        programdata = ProgramSerializer(
            Program.objects.in_menu().order_by("title").exclude(location=True),
            many=True,
        ).data
...
def locations(request):
    ...
    if locs is None:
        locs = ProgramSerializer(
            Program.objects.in_menu().order_by("title").filter(location=True),
            many=True,
        ).data
