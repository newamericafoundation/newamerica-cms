...
class PreviewView(APIView):
    def get(self, request):
        ...
        if content_type.model == "report":
            ...
            about_page_data["subpages"] = AboutPageSerializer(
                ProgramAboutPage.objects.descendant_of(page_being_previewed)
                .live()
                .in_menu(),
                many=True,
            ).data
            ...
            about_page = (
                ProgramAboutHomePage.objects.child_of(program).live().first()
            )
            ...
            for subpage in (
                ProgramAboutPage.objects.descendant_of(about_page)
                .live()
                .in_menu()
            ):
                if subpage.pk == page_being_previewed.pk:
                    about_page_data["subpages"].append(
                        AboutPageSerializer(page_being_previewed).data
                    )
                else:
                    about_page_data["subpages"].append(
                        AboutPageSerializer(subpage).data
                    )
