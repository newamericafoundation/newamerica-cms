...
class Carousel extends Component {
  ...
  componentWillMount(){
    ...
    this.itemGroups = this.splitItems(this.props.items);
    this.hasNext = position < this.itemGroups.length;
    this.hasPrevious = position > 0;
  }

  componentWillUpdate(nextProps, nextState){
    let { position } = nextState;
    this.itemGroups = this.splitItems(nextProps.items);
...
const EventCarousel = ({ response }) => (
  <div className="program-event-carousel">
    {response.results && <Carousel items={response.results} />}
  </div>
)
