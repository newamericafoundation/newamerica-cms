...
export default class Reel extends Panel {
  ...
  reelContent = (p, i) => {
    ...
    if (setHTML)
      return (
        <div
          className={`home__reel__frame ${index === i ? 'active' : ''}`}
          key={`frame-${i}`}
        >
          <div
            className="home__reel__frame__wrapper"
          ...
          <div className={`row gutter-20 home__reel ${panelName}`}>
            <div className="col-md-6">
              <div className="home__panel__promo__text">
                <h1 className="promo margin-top-0 margin-bottom-15">
                  {data.heading[0]}
                </h1>
                <p className="margin-bottom-35 margin-top-15">
            ...
            <div className="col-md-5 push-md-1">
              <div
                className={`home__reel__frames ${pause ? 'pause' : ''}`}
                style={{ top: `${-index * 100}%` }}
              >
                {data.paragraph.map((p, i) => this.reelContent(p, i))}
