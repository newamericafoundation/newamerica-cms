...
class SubscriptionProgramRelationship(models.Model):
    ...
    panels = [FieldPanel("subscription_segment"), FieldPanel("alternate_title")]
...
class AbstractProgram(RoutablePageMixin, Page):
    ...
    fellowship = models.BooleanField(
        null=True, blank=True, help_text="Select if this is a fellowship program"
    )
...
class Program(AbstractProgram):
    ...
    def get_context(self, request):
        ...
        if getattr(request, "is_preview", False):
            ...
            program_data = (
                newamericadotorg.api.program.serializers.ProgramDetailSerializer(
...
class Project(Subprogram):
    ...
    edit_handler = TabbedInterface(
        [
            ...
            ObjectList(
                Subprogram.settings_panels, heading="Settings", classname="settings"
            ),
        ]
    )
